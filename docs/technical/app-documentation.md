...
### 17.7 Billing Heartbeat and Plan History

`utils/billing-cron.ts`'s `runMonthlyBilling` writes an `AppConfig` row (`key: BILLING_HEARTBEAT_KEY`, a constant exported from `billing.controller.ts` - `billing-cron.ts` imports it from there, not the reverse, since it already imports `buildBillForPeriod` from there and a reverse import would make the two files circular) on every tick, whether or not it makes a bill. `GET /billing/heartbeat` reads that back: `staleHeartbeat` is true once the gap since the last tick passes ~3 hours (the job ticks hourly, so that means the server itself has likely been down, not just "nothing to bill"), and separately reports how many of today's active stores have a usage bill for the last finished period - informational, not a pass/fail, since a store with zero sales that month legitimately gets no bill.

The moment `runMonthlyBilling` creates one or more bills, `notifyNewBills()` emails each affected store's own active Super Admin(s) (`userStoreRole` joined to `user` filtered to `role: SUPER_ADMIN, isActive: true, email: {not: null}`) their invoice via the existing `sendBillingInvoiceEmail`. This is separate from the pre-existing manual "Notify Super Admin" button (`sendBillingReport`), which resends every record in a chosen period - paid ones included - on demand; the new path fires automatically, once, only for a bill just created.

`GET /billing/stores/:storeId/plan-history` reads `AuditLog` for `action: 'STORE_BILLING_UPDATE'` entries with that `entityId` (written by the existing `updateStoreBilling`) rather than a new table - the log already carries who/when/before/after for every fee or plan change, so no schema change was needed for this batch.

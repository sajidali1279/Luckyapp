...
export default function Support() {
  ...
  const s: Record<string, React.CSSProperties> = {
    ...
    filterBar:    { background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '16px 20px', marginBottom: 20, display: 'flex', gap: 12, flexWrap: 'wrap' as const, alignItems: 'center' },
    ...
    listBody:     { maxHeight: 'calc(100vh - 380px)', overflowY: 'auto' as const },
    ...
    threadSubj:   { fontWeight: 600, fontSize: 14, color: '#111', marginBottom: 4, whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis' },
    threadMeta:   { display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' as const },
    ...
    detailPanel:  { background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column' as const, maxHeight: 'calc(100vh - 220px)' },
    ...
    detailMeta:   { display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' as const },
    detailActions:{ display: 'flex', gap: 8, marginTop: 12 },
    messages:     { flex: 1, overflowY: 'auto' as const, padding: '20px 24px', display: 'flex', flexDirection: 'column' as const, gap: 16 },
    ...
    replyInput:   { flex: 1, padding: '10px 14px', border: '1px solid #e5e7eb', borderRadius: 10, fontSize: 14, resize: 'vertical' as const, minHeight: 60, maxHeight: 140, outline: 'none', fontFamily: 'inherit' },
    ...
    overlay:      { position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 },
    modal:        { background: '#fff', borderRadius: 16, padding: '28px 32px', width: 540, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto' as const },
    ...
    select:       { width: '100%', padding: '10px 12px', border: '1px solid #d1d5db', borderRadius: 8, fontSize: 14, background: '#fff', outline: 'none', boxSizing: 'border-box' as const },
    textarea:     { width: '100%', padding: '10px 12px', border: '1px solid #d1d5db', borderRadius: 8, fontSize: 14, resize: 'vertical' as const, minHeight: 100, outline: 'none', boxSizing: 'border-box' as const, fontFamily: 'inherit' },
                ...
                return (
                  ...
                  >
                    ...
                    <div style={s.threadMeta}>
                      ...
                      <span style={{ ...s.chip, color: t.status === 'OPEN' ? '#059669' : '#6b7280', background: t.status === 'OPEN' ? '#f0fdf4' : '#f9fafb', borderColor: t.status === 'OPEN' ? '#a7f3d0' : '#e5e7eb' }}>
                        {t.status === 'OPEN' ? '● Open' : '✓ Resolved'}
                      </span>
                      {isDevAdmin && <span style={{ fontSize: 12, color: '#9ca3af' }}>{t.fromName}</span>}
                    </div>
                  </div>
                );
              })}
                ...
                <>
                  <div style={s.detailHeader}>
                    ...
                    <div style={s.detailMeta}>
                      ...
                      <span style={{ ...s.chip, color: CATEGORY_CONFIG[detail.category]?.color || '#6b7280', background: '#f9fafb', borderColor: '#e5e7eb' }}>
                        {CATEGORY_CONFIG[detail.category]?.emoji} {CATEGORY_CONFIG[detail.category]?.label || detail.category}
                      </span>
                      <span style={{ ...s.chip, color: detail.status === 'OPEN' ? '#059669' : '#6b7280', background: detail.status === 'OPEN' ? '#f0fdf4' : '#f9fafb', borderColor: detail.status === 'OPEN' ? '#a7f3d0' : '#e5e7eb' }}>
                        {detail.status === 'OPEN' ? '● Open' : '✓ Resolved'}
                      </span>
                      <span style={{ fontSize: 12, color: '#9ca3af' }}>opened {timeAgo(detail.createdAt)}</span>
                    </div>
                    {isDevAdmin && (
                  ...
                  {detail.status === 'RESOLVED' && !isDevAdmin && (
                    <div style={{ padding: '16px 24px', borderTop: '1px solid #f3f4f6', textAlign: 'center', color: '#6b7280', fontSize: 14 }}>
